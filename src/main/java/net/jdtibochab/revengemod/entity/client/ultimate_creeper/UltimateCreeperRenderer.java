...
@OnlyIn(Dist.CLIENT)
public class UltimateCreeperRenderer extends MobRenderer<UltimateCreeperEntity, UltimateCreeperModel<UltimateCreeperEntity>> {

    public UltimateCreeperRenderer(EntityRendererProvider.Context context) {
        super(context, new UltimateCreeperModel<>(context.bakeLayer(ModelLayers.CREEPER)), 0.5F);
        this.addLayer(new UltimateCreeperEyesLayer<>(this));
        this.addLayer(new UltimateCreeperPowerLayer(this, context.getModelSet()));
    }

    @Override
    public ResourceLocation getTextureLocation(UltimateCreeperEntity pEntity) {
        return new ResourceLocation(RevengeMod.MOD_ID, "textures/entity/ultimate_creeper.png");
    }

    protected void scale(UltimateCreeperEntity pLivingEntity, PoseStack pMatrixStack, float pPartialTickTime) {
    ...
    protected float getWhiteOverlayProgress(UltimateCreeperEntity pLivingEntity, float pPartialTicks) {
